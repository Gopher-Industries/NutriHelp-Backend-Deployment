...
class SignupService {
  ...
  async signupPublicTable({ userUuid, name, email, password, contactNumber, address, ip, userAgent }) {
    ...
    if (existingUsers.length > 0) {
      ...
      throw new ServiceError(400, 'Email is already registered', { code: 'EMAIL_EXISTS' });
    }
  ...
  async signupAuthTable({ name, email, password, contactNumber, address, ip, userAgent }) {
    ...
    if (error) {
      ...
      if (message.includes('password') || message.includes('weak')) {
        throw new ServiceError(400, error.message, { code: 'WEAK_PASSWORD' });
      }
    ...
    if (data.session?.access_token) {
      try {
        ...
        await authed.from('profiles').upsert(
          { id: userId, email, name, contact_number: contactNumber || null, address: address || null },
          { onConflict: 'id' }
        );
      } catch (profileError) {
