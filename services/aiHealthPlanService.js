...
const { retrieve } = require("../routes/ai/chatbot");
...
/**
 * Generate an 8-week health plan using the Groq LLM + RAG context.
 * @param {object|object[]} medical_report
 * @param {object} health_goal  { days_per_week, target_weight?, workout_place? }
 * @returns {Promise<{suggestion, weekly_plan, progress_analysis}>}
 */
async function generatePlan(medical_report, health_goal) {
  const numWeeks = 8;
  ...
  return enforceSchema(forceJson(output), numWeeks);
}
